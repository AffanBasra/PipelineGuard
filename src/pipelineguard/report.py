...
class ReportData:
    ...
    # `source` fixed the header; this fixes the prose. A scan of an uploaded
    # file forwards nothing, writes no audit trail and has no review topic, so
    # sentences describing those are false about it. See decisions.md.
...
# --------------------------------------------------------------------------- #
# Passages that are true of a stream run and false of a file scan.
#
# Both renderers share them. Keeping them here rather than inline stops the two
# documents drifting into disagreeing about the same system.
# --------------------------------------------------------------------------- #
def _system_properties_scope(data: ReportData) -> str:
    if data.from_stream:
        return ("What the pipeline does, and why each regime cares. The "
                "behaviour is a statement of fact about this system; the two "
                "bases explain significance and are not assertions that an "
                "obligation has been discharged.")
    return ("What the PipelineGuard *pipeline* does, and why each regime "
            "cares. It is here because it explains why detection behaves the "
            "way it does -- the scan reported above ran the same detectors. "
            "None of it describes what happened to the scanned file, which was "
            "read in memory, never stored and never forwarded anywhere.")


def _coverage_limit(data: ReportData) -> str:
    if data.from_stream:
        return ("- **This report describes what entered the pipeline, not "
                "what existed.** Whether every record reached it is a question "
                "about Kafka offsets and consumer lag, which this audit trail "
                "cannot answer. It is not evidence that a source system was "
                "scanned completely.")
    return ("- **This report covers the rows in one file, nothing else.** It "
            "says what was found in what was submitted, and is not evidence "
            "that any source system was scanned completely.")


def _retention_limit(data: ReportData) -> str:
    if data.from_stream:
        return ("- **Delivery is at-least-once.** A record processed twice is "
                "upserted on its message id, so it is counted once here -- but "
                "the guarantee is idempotent recording, not exactly-once "
                "processing.")
    return ("- **Nothing was recorded.** This scan wrote no audit trail and "
            "no database row. These figures exist only in this document, and "
            "cannot be re-derived once the scanned text is gone.")


_EXAMPLE_REPO = "https://github.com/AffanBasra/PipelineGuard"

# The banner carries the repo link because the file outlives the page. A reader
# who wants to check a figure, or cite this, needs somewhere to go from here.
_EXAMPLE_BANNER = (
    "> **This is an example, not a report of your data.** It is a stored run "
    "of the PipelineGuard pipeline over synthetic records, shipped with the "
    "demo because the demo has no pipeline or database behind it.\n"
    ">\n"
    f"> **Source project:** PipelineGuard — <{_EXAMPLE_REPO}>. This file was "
    "generated by `scripts/make_sample_summary.py` from a live pipeline run. "
    "Cite the repository rather than this document: the figures below describe "
    "one synthetic run and are not a published result."
)


def stamp_example(markdown_text: str) -> str:
    """Mark a stored report as an example, inside the file itself.

    A caption on a page does not travel with a download. Whoever opens the PDF
    tomorrow has only what is printed in it, so the provenance has to be there.
    """
    out: list[str] = []
    for line in markdown_text.splitlines():
        if not out and line.startswith("# "):
            out += [line.rstrip() + " (example)", "", _EXAMPLE_BANNER]
        elif line.startswith("**Source:**"):
            out.append(line.rstrip() + " -- a stored example run")
        else:
            out.append(line)
    return "\n".join(out).rstrip() + "\n"
...
def render(data: ReportData) -> str:
    ...
    if data.entities:
        ...
        lines += [
            ...
            "no value is stored with which to link them. That is a deliberate "
            "consequence of not retaining personal data.",
        ]
...
def _quarantine_summary(data: ReportData) -> list[str]:
    ...
    lines += [
        ...
        "document is meant to be shareable. "
        + ("The queue itself is held in the `txn.quarantine` topic, where a "
           "reviewer can work through it." if data.from_stream else
           "Nothing was routed anywhere: this was a one-off scan, and the "
           "flagged rows are marked in the tool that produced this."),
    ]
    return lines
...
def render_summary(data: ReportData) -> str:
    ...
    lines: list[str] = [
        ...
        + ("the pipeline saw and did" if data.from_stream
           else "one scan of one file found")
        + ". It is not a compliance determination and not legal advice.",
        "",
        "## 1. Processing scope",
        "",
    ]
    ...
    lines += [
        ...
        "## 5. Limitations",
        "",
        ("- **This covers what reached the pipeline, not what exists.** "
         "Whether every record arrived is a question this audit trail cannot "
         "answer." if data.from_stream else
         "- **This covers the rows in one file, nothing else.** It is not "
         "evidence that any source system was scanned completely."),
        ...
        ("- **Delivery is at-least-once.** A record seen twice is counted "
         "once, but that is idempotent recording, not exactly-once "
         "processing." if data.from_stream else
         "- **Nothing was recorded.** This scan wrote no audit trail and no "
         "database row. These figures exist only in this document."),
    ]
    ...
    if not data.from_stream:
        lines += [_system_properties_scope(data), ""]
    for prop in compliance.SYSTEM_PROPERTIES:
        lines += [
